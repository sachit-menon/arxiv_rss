...
def get_arxiv_query(author_name: str)->str:
    ...
    abbrev_name[0] = abbrev_name[0][0]
    return f'http://export.arxiv.org/api/query?search_query=au:%22{author_name_list[0]}+{author_name_list[1]}%22+OR+au:%22{abbrev_name[0]}+{abbrev_name[1]}%22&sortBy=submittedDate&sortOrder=descending&max_results=100'.replace('&', '&amp;')
